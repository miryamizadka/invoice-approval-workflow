...
| Auth | Issue/verify JWTs (register, login), seed demo Approver/Admin accounts when `SEED_DEMO_USERS=true` (N1) | REST (external) | state | users |
...
| Frontend | Static HTML/CSS/vanilla JS | No framework, no build step (M7, ADR-010) - mirrors the REST API directly |
...
| Gateway | Approval | REST | Sync | Escalation queue + approve/reject/request-info/additional-info (F4/F5) |
...
The policy text and autonomy thresholds live in a Dapr configuration store (`configuration.redis`, scoped to Decision), not hard-coded Python. Decision reads them **once at startup** (fetch-once, not live hot-reload - see ADR-009) via its FastAPI lifespan hook, the same pattern already used for Payment's budget seeding and Audit's schema creation. A controller changes a value by writing directly to the backing store (`redis-cli SET ceiling 500` - Dapr's Configuration API is read/subscribe-only from the app's side, there is no write call) and running `docker compose restart decision` - **no code change and no image rebuild**, but the change takes effect on the next restart, not instantaneously. Every field falls back independently to `DEFAULT_THRESHOLDS`/`policy.md` if the store has nothing set for it or is unreachable at startup - the service always starts and functions correctly, configured or not (verified live: booting against an empty store logs a graceful fallback, never a crash). The numbers in the store are the same ones enforced by the router, and must match PRODUCT-DILEMMA.md.
